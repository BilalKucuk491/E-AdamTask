...
const server = http.createServer(function (req, res) {
    switch (req.url.toLowerCase()) {
        case "/": return res.end("200");
        case "/getcars": return getCars(req, res)
        case "/savecar": return saveCar(req, res)
        case "/sellcar": return sellCar(req, res)
        default: return getCustomPage(req, res)
    }
})
server.listen(8001);
...
const getCars = (req, res) => {
    ...
    res.end(JSON.stringify(JSON.parse(jsonString)) || JSON.stringify({ error: "error 1" }))
}
...
const saveCar = (req, res) => {
    ...
    req.on('end', function () {
        ...
        let fields = ["categories", "brand", "series", "year", "color", "km", "engineCapacity", "plate"]
...
const sellCar = (req, res) => {
    ...
    req.on('end', function () {
        ...
        if (!body.hasOwnProperty("plate") || body.plate == null || body.plate == "") {
            res.end(JSON.stringify({ error: field + " is not exist"}))
            return
        }
